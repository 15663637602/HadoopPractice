...
public class Mapper2 extends Mapper<LongWritable, Text, Text, Text>{
	...
	protected void map(LongWritable key, Text value, Context context)
		...
		for(String column_value_1 : column_value_matrix1){
			String score = column_value_1.split("_")[1];
		...
		for(String line : cacheList){
			...
			//计算右侧矩阵每行的空间距离  与denominator相乘就是 -> 分母
			double denominator2 = 0;
			for(String column_value_2 : column_value_matrix2){
				String score2 = column_value_2.split("_")[1];
				denominator2 += Double.valueOf(score2)*Double.valueOf(score2);
			}
			...
			outValue.set(row_matrix2+"_"+df.format(cos));// 保留两位小数
